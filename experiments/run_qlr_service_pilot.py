#!/usr/bin/env python3
"""Run the fixed five-service QLR pilot without invoking UA-MSTCN."""
...
def build_selection_profile(
    ...
    fixed_app_to_role = {app_du: role for role, app_du in FIXED_SERVICE_ROLES.items()}
    selection_rule = (
        "fixed five-service pilot chosen from train+validation prefix load/burst quadrants; "
        "full-span summary descriptors are not used for selection"
    )
    ...
    for app_du, group in service_df.groupby("app_du"):
        ...
        train_prefix, _, split_idx = split_series(values, train_ratio, valid_ratio)
        load_mean = float(np.mean(train_prefix))
        burst_std = float(np.std(train_prefix, ddof=1)) if len(train_prefix) > 1 else 0.0
        burst_cv = burst_std / max(load_mean, 1e-9)
        peak_to_mean = float(np.max(train_prefix) / max(load_mean, 1e-9)) if len(train_prefix) else 0.0
        ...
        profile_rows.append(
            {
                ...
                "app_du": app_du,
                "prefix_rows": int(len(train_prefix)),
    ...
    if not all_prefix_df.empty:
        load_median = float(all_prefix_df["load_mean_prefix"].median())
        burst_median = float(all_prefix_df["burst_cv_prefix"].median())
        profile_df["all_service_load_median_prefix"] = load_median
        profile_df["all_service_burst_cv_median_prefix"] = burst_median
        profile_df["eligible_service_count_prefix_only"] = int(len(all_prefix_df))

    role_order = list(FIXED_SERVICE_ROLES)
    ...
    gate_failures = []
    if len(profile_df) != len(FIXED_SERVICE_ROLES):
        gate_failures.append(f"selection trace has {len(profile_df)} services, expected {len(FIXED_SERVICE_ROLES)}")
    missing_roles = sorted(set(FIXED_SERVICE_ROLES) - set(profile_df["selection_role"]))
    if missing_roles:
        gate_failures.append(f"selection trace missing roles: {', '.join(missing_roles)}")
    return profile_df, gate_failures


def service_request_cores(service_meta: pd.Series) -> tuple[float, str, bool]:
...
def evaluate_service(
    ...
    selection_role: str,
    service_df: pd.DataFrame,
    service_meta: pd.Series,
    config: dict,
    alpha: float,
    solver: str,
) -> tuple[list[dict], list[dict], list[dict], list[dict], list[dict], list[str], bool]:
    ...
    series = service_df.sort_values("minute_index")["service_cpu_used_cores_proxy"].to_numpy(dtype=float)
    ...
    return forecast_rows, prediction_rows, policy_rows, policy_series_rows, gate_failures, request_fallback_used
...
def write_report(
    ...
    report_lines = [
        ...
        f"- metrics_csv_updated: `{metrics_hash_before != metrics_hash_after}`",
    ...
    if fallback_services:
        ...
        report_lines.append("No selected service required fallback from `median_container_request_cores` to `mean_container_request_cores`.")
    report_lines.extend(
        [
            "",
    ...
    report_path = output_dir / "qlr_service_pilot_report.md"
...
def main() -> None:
    ...
    service_csv = args.service_csv or (repo_root / "data" / "processed" / "service_cohort_broad_v2018.csv")
    summary_csv = args.service_summary_csv or (
        repo_root / "data" / "processed" / "service_cohort_broad_summary_v2018.csv"
    )
    service_csv = resolve_workbench_path(service_csv, repo_root)
    summary_csv = resolve_workbench_path(summary_csv, repo_root)
    output_dir = args.output_dir or (repo_root / "experiments" / "results" / "qlr_service_pilot")
    output_dir.mkdir(parents=True, exist_ok=True)

    metrics_path = resolve_workbench_path(config["output"]["metrics_csv"], repo_root)
    metrics_hash_before = None
    if metrics_path.exists():
        import hashlib

        metrics_hash_before = hashlib.sha256(metrics_path.read_bytes()).hexdigest()

    selected_apps = set(FIXED_SERVICE_ROLES.values())
    ...
    selection_df.to_csv(output_dir / "service_selection_profile.csv", index=False)
    ...
    if args.num_workers != 1:
        gate_failures.append("service pilot must be run with --num-workers 1 for this gate")

    for role, app_du in FIXED_SERVICE_ROLES.items():
        app_df = selected_service_df[selected_service_df["app_du"] == app_du].copy()
        if app_df.empty:
            gate_failures.append(f"selected service {app_du} has no rows in service CSV")
            continue
        service_forecast, service_predictions, service_policy, service_policy_series, service_failures, fallback_used = (
            evaluate_service(
                app_du,
                role,
                app_df,
                summary_lookup.loc[app_du],
                config,
                args.alpha,
                args.solver,
            )
        )
        forecast_rows.extend(service_forecast)
        prediction_rows.extend(service_predictions)
        policy_rows.extend(service_policy)
        policy_series_rows.extend(service_policy_series)
        gate_failures.extend(service_failures)
        if fallback_used:
    ...
    if len(set(forecast_df["app_du"])) != 5:
        gate_failures.append("forecast artifacts do not contain exactly five services")
    ...
    metrics_hash_after = metrics_hash_before
    if metrics_path.exists():
        import hashlib

        metrics_hash_after = hashlib.sha256(metrics_path.read_bytes()).hexdigest()
    if metrics_hash_before != metrics_hash_after:
        gate_failures.append("metrics.csv changed during service pilot")
    ...
    if gate_failures:
        raise SystemExit(f"QLR service pilot gate failed; see {report_path}")
